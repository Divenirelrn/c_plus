...
* 
*	//全局函数重载<<号
*	ostream &operator<<(ostream &out, Person &p) //函数返回out的引用，因此可以链式编程;out参数为引用形式，因为全局只能有一个
*	{
*		out << p.m_A << m_B;
*		return out;
*	}
...
* 
*	//重载左移运算符
*	ostream& oprator<<(ostream& cout, MyInteger myint) //myint不能设为引用类型，否则无法接收后置++(后置++里的局部变量temp不能作为引用返回)
*	{ 
*		cout << myint.m_Num;
*		return cout;
*	}
